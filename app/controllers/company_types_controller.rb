class CompanyTypesController < ApplicationController
  ...
  def create
    if params[:id]
      ...
      @company_type.name=params[:name]
      @company_type.status=1
      if @company_type.save
        render :json => @company_type
      else
        render :json => @company_type.errors.full_messages, status: :unprocessable_entity
      end
    else
      @company_type = CompanyType.new
      @company_type.name=params[:company_type]
      @company_type.status=1
      if @company_type.save
        render :json => @company_type
      else

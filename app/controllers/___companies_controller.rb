class CompaniesController < ApplicationController
  ...
  def getbyName
    ...
    if(@company_names_array.count >  50.to_i * @page.to_i)
      @to = (50.to_i) * @page.to_i - 1.to_i
    else
      @to = @company_names_array.count - 1
    end

    for current_iteration_number in @from..@to do
      #@array[current_iteration_number] = (@company_names_array[current_iteration_number])
      @array.push(@company_names_array[current_iteration_number])
    end
    @result = Hash.new
    @result[:records] = @array
    @result[:total] = @company_names_array.count
    render :json =>@result
